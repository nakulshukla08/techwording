...
public class EmbeddedKafkaAwaitilityTest {
	...
	@Autowired
	private EmbeddedKafkaAwaitilityTest.Producer producer;

	@Autowired
	private EmbeddedKafkaAwaitilityTest.Consumer consumer;
		...
		waitAtMost(5, TimeUnit.SECONDS)
			.untilAsserted(() -> {
				then("payload").isEqualTo(
					EmbeddedKafkaAwaitilityTest.this.consumer.getRecievedMessage());
			});
	}
